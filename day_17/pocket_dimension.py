    ...
    for x in list(dimension.keys()):
        for y in list(dimension[x].keys()):
            for z in list(dimension[x][y].keys()):
                if dimension[x][y][z][STATE] is ACTIVE:
                    active_neighbors = count_active_neighbors(dimension, x, y, z)
                    if not (2 <= active_neighbors <= 3):
                        dimension[x][y][z][POTENTIAL_ENERGY] = -1

    # Commit Potential Energy
    for x in dimension:
        for y in dimension[x]:
            for z in dimension[x][y]:
                if dimension[x][y][z][STATE] is INACTIVE and dimension[x][y][z][POTENTIAL_ENERGY] == 3:
                    dimension[x][y][z][STATE] = ACTIVE
                elif dimension[x][y][z][STATE] is ACTIVE and dimension[x][y][z][POTENTIAL_ENERGY] == -1:
                    dimension[x][y][z][STATE] = INACTIVE
                dimension[x][y][z][POTENTIAL_ENERGY] = 0

    return dimension


def count_active_neighbors(dimension, x, y, z):
        ...
        for y_idx in y_range:
            ...
            for z_idx in z_range:
                if z_idx not in dimension[x_idx][y_idx].keys():
                    dimension[x_idx][y_idx][z_idx] = [INACTIVE, 0]
                if x_idx is x and y_idx is y and z_idx is z:
                    continue
                # COUNT AND ADD POTENTIAL
                if dimension[x_idx][y_idx][z_idx][STATE] is ACTIVE:
                    active_neighbors += 1
                elif dimension[x_idx][y_idx][z_idx][POTENTIAL_ENERGY] != -1:
                    dimension[x_idx][y_idx][z_idx][POTENTIAL_ENERGY] += 1
